...
for i in range(int(n)):
    p1 = ((xp-x[i+1])/(x[i]-x[i+1]))*f[i] + ((xp-x[i])/(x[i+1]-x[i]))*f[i+1]
    fp.append(p1)
print(fp)
ff = f[6]
fp.append(ff)
...
if int(n) == 1:
    l0 = (xp - x[1])/(x[0]-x[1])
    l1 = (xp - x[0])/(x[1]-x[0])
    p = np.multiply(l0,fp[0]) + np.multiply(l1,fp[1])
else:
    if int(n) == 2:
        l0 = (xp-x[1])*(xp-x[2])/(x[0]-x[1])*(x[0]-x[2])
        l1 = (xp-x[0])*(xp-x[2])/(x[1]-x[0])*(x[1]-x[2])
        l2 = (xp-x[0])*(xp-x[1])/(x[2]-x[0])*(x[2]-x[1])
        p = np.multiply(l0,fp[0]) + np.multiply(l1,fp[1]) + np.multiply(l2,fp[2])
    else:
        if int(n) == 6:
            ...
            l5 = l5u/l5d
            l6 = l6u/l6d
            p = np.multiply(l0, fp[0]) + np.multiply(l1, fp[1]) + np.multiply(l2, fp[2]) + np.multiply(l3, fp[3]) + np.multiply(l4, fp[4]) + np.multiply(l5, fp[5]) + np.multiply(l6, fp[6])
